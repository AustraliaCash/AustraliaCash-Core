#!/usr/bin/env python3
# Copyright (c) 2021-2022 The AustraliaCash Core developers
...
class P2PPolicyTests(BitcoinTestFramework):
    ...
    def setup_network(self):
        ...
        # a AustraliaCash Core node that behaves similar to mainnet policies
    ...
    def run_test(self):
        ...
        ### test constants ###
        koinu = Decimal("0.00000001")          # 1 Koinu expressed in AUS
        ten = Decimal("10.0")                  # uniform 10 AUS seed moneys
        ...
        # create a bunch of UTXO with seed money from the AustraliaCash Core wallet
        for i in range(20):
        ...
        # test legacy output of 1 AUS output and 1 AUS fee
    ...
    # spend seed money with a key not in the AustraliaCash Core wallet.
    def spend_utxo(self, output, expected_size, retries=0):
        # construct the transaction using AustraliaCash Core raw tx APIs
        ...
        # and send the tx from there rather than from AustraliaCash Core, to test
