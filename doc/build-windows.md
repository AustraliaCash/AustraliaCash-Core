...
Below are some notes on how to build CyberDollar Core for Windows.
...
Note that for WSL v1 the CyberDollar Core source path MUST be somewhere in the default mount file system, for
example /usr/src/cyberdollar, AND not under, for example, /mnt/d/cyberdollar. 
...
way. This will install to `c:\workspace\cyberdollar`, for example:

    make install DESTDIR=/mnt/c/workspace/cyberdollar
