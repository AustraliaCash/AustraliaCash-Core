...
`gitian-build.sh` is a standalone script, it can be downloaded and run outside of CyberDollar Core repository.
...
```bash
gpg --detach-sign ${VERSION}-linux/${SIGNER}/cyberdollar-linux-build.assert
gpg --detach-sign ${VERSION}-win/${SIGNER}/cyberdollar-win-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/cyberdollar-osx-build.assert
```
...
Gitian signatures for each release are added to https://github.com/cyberdollar/gitian.sigs.

`gitian-build.sh` will create signatures inside `gitian-output/sigs/` folder. Create a pull request to [cyberdollar/gitian.sigs](https://github.com/cyberdollar/gitian.sigs) to publish your signatures, the `.assert` and `.assert.sig` files.

**When your PR is merged, you will be recorded for all future history as a *Gitian Builder of CyberDollar Core*!**
