...
`gitian-build.sh` is a standalone script, it can be downloaded and run outside of AustraliaCash Core repository.
...
```bash
gpg: skipped "acashtoshi": secret key not available
```
...
```bash
gpg --detach-sign ${VERSION}-linux/${SIGNER}/australiacash-linux-build.assert
gpg --detach-sign ${VERSION}-win/${SIGNER}/australiacash-win-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/australiacash-osx-build.assert
```
...
Gitian signatures for each release are added to https://github.com/australiacash/gitian.sigs.

`gitian-build.sh` will create signatures inside `gitian-output/sigs/` folder. Create a pull request to [australiacash/gitian.sigs](https://github.com/australiacash/gitian.sigs) to publish your signatures, the `.assert` and `.assert.sig` files.

**When your PR is merged, you will be recorded for all future history as a *Gitian Builder of AustraliaCash Core*!**
