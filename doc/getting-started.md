...
The AustraliaCash Core installation provides the `australiacash-cli` tool to interact with the JSON-RPC from the command line, and the interface is exposed over HTTP on port `1986`, so that other tools and libraries can interact with it.
