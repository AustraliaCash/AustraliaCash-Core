AustraliaCash version 0.7.0 is now available for download at:
  http://sourceforge.net/projects/australiacash/files/AustraliaCash/australiacash-0.7.0/

We recommend that everybody running prior versions of australiacashd/AustraliaCash-Qt
...
source-only tarballs/zipballs directly from there:
  https://github.com/australiacash/australiacash/tarball/v0.7.0  # .tar.gz
  https://github.com/australiacash/australiacash/zipball/v0.7.0  # .zip
...
in your terminal, then install the australiacash-qt package:
  sudo apt-get install australiacash-qt
...
(on Mac) or
Code:
australiacashd/australiacash-qt
(on Linux).
...
Core australiacash handling and blockchain database
...
* Ensure a changed australiacash unit immediately updates all GUI elements that use units
...
* (Windows only): enable ASLR and DEP for australiacash-qt.exe
* (Windows only): add meta-data to australiacash-qt.exe (e.g. description)
...
* Bash programmable completion for australiacashd(1)
