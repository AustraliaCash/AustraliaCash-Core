AustraliaCash Core version 0.11.0 is now available from:

  <https://australiacash.org/bin/australiacash-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer AUS/kB fee to be
...
For example, add the following to `australiacash.conf`:
...
related to the blockchain in the australiacash system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows AustraliaCash Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that AustraliaCash Core will 
...
For an example script see [australiacash-submittx](https://github.com/laanwj/australiacash-submittx).
...
- #5334 `cf87536` libaustraliacashconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new australiacash-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only AustraliaCashAmountField
- #5753 `0f44672` Add australiacash logo to about screen
...
- #5809 `46bfbe7` Add australiacash-cli man page
...
- #5849 `41259ca` contrib/init/australiacashd.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename AustraliaCash-Qt.app to AustraliaCash-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for australiacash-tx)
