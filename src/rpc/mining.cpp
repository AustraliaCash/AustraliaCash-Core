...
static UniValue generatetoaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "2. address      (string, required) The address to send the newly generated australiacash to.\n"
...
// NOTE: Unlike wallet RPC (which use AUS values), mining RPCs follow GBT (BIP 22) in using satoshi amounts
...
static UniValue getblocktemplate(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            ...
            "    https://github.com/australiacash/bips/blob/master/bip-0022.mediawiki\n"
            "    https://github.com/australiacash/bips/blob/master/bip-0023.mediawiki\n"
            "    https://github.com/australiacash/bips/blob/master/bip-0009.mediawiki#getblocktemplate_changes\n"
            "    https://github.com/australiacash/bips/blob/master/bip-0145.mediawiki\n"
    ...
    if (g_connman->GetNodeCount(CConnman::CONNECTIONS_ALL) == 0)
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "AustraliaCash is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "AustraliaCash is downloading blocks...");
...
static UniValue submitblock(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2) {
        throw std::runtime_error(
            ...
            "See https://en.australiacash.it/wiki/BIP_0022 for full specification.\n"
