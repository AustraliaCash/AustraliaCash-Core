This directory contains the AustraliaCashQT graphical user interface (GUI). It uses the cross-platform framework [Qt](https://www1.qt.io/developers/).
...
### australiacashgui.(h/cpp)

Represents the main window of the AustraliaCash UI.
...
ClientModel is used by the main application `australiacashgui` and several models like `peertablemodel`.
...
Used to process BIP21 and BIP70 (see https://github.com/australiacash/australiacash/pull/11622) payment URI / requests. Also handles URI based application switching (e.g. when following a australiacash:... link from a browser).
...
* UI elements like AustraliaCashAmountField, which inherit from QWidget.
* `australiacashstrings.cpp`: automatically generated
* `australiacashunits.(h/cpp)`: AUS / mAUS / etc handling
* `callback.h`
...
* don't change `local/australiacash_en.ts`; this happens [automatically](/doc/translation_process.md#writing-code-with-translations)
...
10. Start debugging with Qt Creator (you might need to the executable to "australiacash-qt" under "Run", which is where you can also add command line arguments)
