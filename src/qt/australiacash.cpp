...
{
    return QCoreApplication::translate("australiacash-core", psz).toStdString();
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. australiacash_de.qm (shortcut "de" needs to be defined in australiacash.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. australiacash_de_DE.qm (shortcut "de_DE" needs to be defined in australiacash.qrc)
 ...
 */
class AustraliaCashCore: public QObject
{
    Q_OBJECT
public:
    explicit AustraliaCashCore(interfaces::Node& node);
...
class AustraliaCashApplication: public QApplication
{
    Q_OBJECT
public:
    explicit AustraliaCashApplication(interfaces::Node& node, int &argc, char **argv);
...
#include <qt/australiacash.moc>

AustraliaCashCore::AustraliaCashCore(interfaces::Node& node) :
    QObject(), m_node(node)
{
}

void AustraliaCashCore::handleRunawayException(const std::exception *e)
{
    PrintExceptionContinue(e, "Runaway exception");
    Q_EMIT runawayException(QString::fromStdString(m_node.getWarnings("gui")));
}

void AustraliaCashCore::initialize()
{
    try
    {
...
void AustraliaCashCore::shutdown()
{
    try
    {
...
AustraliaCashApplication::AustraliaCashApplication(interfaces::Node& node, int &argc, char **argv):
...
void AustraliaCashApplication::createOptionsModel(bool resetSettings)
{
...
{
    window = new AustraliaCashGUI(m_node, platformStyle, networkStyle, 0);
...
void AustraliaCashApplication::startThread()
{
    ...
    AustraliaCashCore *executor = new AustraliaCashCore(m_node);
...
void AustraliaCashApplication::addWallet(WalletModel* walletModel)
{
#ifdef ENABLE_WALLET
...
void AustraliaCashApplication::removeWallet()
{
#ifdef ENABLE_WALLET
...
void AustraliaCashApplication::initializeResult(bool success)
{
    ...
    if(success)
    {
        ...
        // australiacash: URIs or payment requests:
...
void AustraliaCashApplication::shutdownResult()
{
...
{
    QMessageBox::critical(0, "Runaway exception", AustraliaCashGUI::tr("A fatal error occurred. AustraliaCash can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}
...
static void SetupUIArgs()
{
    ...
    gArgs.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", AustraliaCashGUI::DEFAULT_UIPLATFORM), true, OptionsCategory::GUI);
}

#ifndef AUSTRALIACASH_QT_TEST
int main(int argc, char *argv[])
{
    ...
    Q_INIT_RESOURCE(australiacash);
    Q_INIT_RESOURCE(australiacash_locale);

    AustraliaCashApplication app(*node, argc, argv);
    ...
    /// 6. Determine availability of data and blocks directory and parse australiacash.conf
    ...
    // australiacash: links repeatedly have their payment requests routed to this process:
