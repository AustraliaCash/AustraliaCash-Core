...
void AustraliaCashGUI::createActions()
{
    ...
    openAction->setStatusTip(tr("Open a australiacash: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible AustraliaCash command-line options").arg(tr(PACKAGE_NAME)));
...
void AustraliaCashGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool AustraliaCashGUI::addWallet(WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool AustraliaCashGUI::removeWallet(WalletModel* walletModel)
{
...
bool AustraliaCashGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
bool AustraliaCashGUI::setCurrentWalletBySelectorIndex(int index)
{
...
void AustraliaCashGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void AustraliaCashGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void AustraliaCashGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        tooltip = tr("%n active connection(s) to AustraliaCash network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void AustraliaCashGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}
...
void AustraliaCashGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void AustraliaCashGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("AustraliaCash"); // default title
...
#ifdef ENABLE_WALLET
void AustraliaCashGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
...
void AustraliaCashGUI::setHDStatus(int hdEnabled)
{
...
void AustraliaCashGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
static bool ThreadSafeMessageBox(AustraliaCashGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void AustraliaCashGUI::toggleNetworkActive()
{
    ...
    menu(0)
{
    ...
    QList<AustraliaCashUnits::Unit> units = AustraliaCashUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const AustraliaCashUnits::Unit unit : units)
    {
        max_width = qMax(max_width, fm.width(AustraliaCashUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (AustraliaCashUnits::Unit u : AustraliaCashUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(AustraliaCashUnits::longName(u)), this);
