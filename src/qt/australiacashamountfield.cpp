...
#include <qt/australiacashamountfield.h>
...
public:
    explicit AmountSpinBox(QWidget *parent):
        ...
        currentUnit(AustraliaCashUnits::AUS),
    ...
    void fixup(QString &input) const
    {
        ...
        if(valid)
        {
            input = AustraliaCashUnits::format(currentUnit, val, false, AustraliaCashUnits::separatorAlways);
    ...
    {
        lineEdit()->setText(AustraliaCashUnits::format(currentUnit, value, false, AustraliaCashUnits::separatorAlways));
        Q_EMIT valueChanged();
    }
    ...
    void stepBy(int steps)
    {
        ...
        val = qMin(qMax(val, CAmount(0)), AustraliaCashUnits::maxMoney());
        setValue(val);
    }
    ...
    QSize minimumSizeHint() const
    {
        if(cachedMinimumSizeHint.isEmpty())
        {
            ...
            int w = fm.width(AustraliaCashUnits::format(AustraliaCashUnits::AUS, AustraliaCashUnits::maxMoney(), false, AustraliaCashUnits::separatorAlways));
...
protected:
    ...
    StepEnabled stepEnabled() const
    {
        ...
        if(valid)
        {
            if(val > 0)
                rv |= StepDownEnabled;
            if(val < AustraliaCashUnits::maxMoney())
                rv |= StepUpEnabled;
        }
        return rv;
    }
...
#include <qt/australiacashamountfield.moc>
...
void AustraliaCashAmountField::unitChanged(int idx)
{
    ...
    int newUnit = unit->itemData(idx, AustraliaCashUnits::UnitRole).toInt();

    amount->setDisplayUnit(newUnit);
}

void AustraliaCashAmountField::setDisplayUnit(int newUnit)
{
