 ...
 * between australiacashd, and australiacash-qt and non-server tools.
*/
...
#ifndef WIN32
    gArgs.AddArg("-pid=<file>", strprintf("Specify pid file. Relative paths will be prefixed by a net-specific datadir location. (default: %s)", AUSTRALIACASH_PID_FILENAME), false, OptionsCategory::OPTIONS);
#else
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/AustraliaCash-Network/australiacash>";
    const std::string URL_WEBSITE = "<https://www.australiacash.global/>";
...
/** Sanity checks
 *  Ensure that AustraliaCash is running in a usable environment with all
...
{
    // Make sure only a single AustraliaCash process is using the data directory.
...
bool AppInitMain()
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", AUSTRALIACASH_CONF_FILENAME)).string());
