...
const char * const AUSTRALIACASH_CONF_FILENAME = "australiacash.conf";
const char * const AUSTRALIACASH_PID_FILENAME = "australiacashd.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "australiacashd -foo=bar
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", AUSTRALIACASH_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", AUSTRALIACASH_PID_FILENAME)));
}
