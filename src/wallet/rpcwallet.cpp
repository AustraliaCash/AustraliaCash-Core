...
UniValue sendmany(const JSONRPCRequest& request)
{
    ...
    BOOST_FOREACH(const string& name_, keys)
    {
        ...
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid AustraliaCash address: ")+name_);
...
UniValue listunspent(const JSONRPCRequest& request)
{
    ...
    if (request.params.size() > 2 && !request.params[2].isNull()) {
        ...
        for (unsigned int idx = 0; idx < inputs.size(); idx++) {
            ...
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid AustraliaCash address: ")+input.get_str());
            if (setAddress.count(address))
...
UniValue bumpfee(const JSONRPCRequest& request)
{
    ...
    // Cyber: Round txSize up to nearest 1kB
    if (txSize % 1024 != 0) {
    ...
    if (totalFee > 0) {
        ...
        // otherwise bump the fee by 1 AUS.
        else {
