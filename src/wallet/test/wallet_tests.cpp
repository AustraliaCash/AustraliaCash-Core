...
BOOST_FIXTURE_TEST_CASE(rescan, TestChain240Setup)
{
	...
	// AustraliaCash: Immature balance varies due to random rewards
    ...
    // after.
    {
        ...
        BOOST_CHECK_EQUAL(response.write(),
            strprintf("[{\"success\":false,\"error\":{\"code\":-1,\"message\":\"Rescan failed for key with creation "
                      ...
                      "by pruning or data corruption (see australiacashd log for details) and could be dealt with by "
...
BOOST_FIXTURE_TEST_CASE(coin_mark_dirty_immature_credit, TestChain240Setup)
{
    ...
    wtx.MarkDirty();
    wallet.AddKeyPubKey(coinbaseKey, coinbaseKey.GetPubKey());
    // AustraliaCash has random rewards so we can only test against maximum
