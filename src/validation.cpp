                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept)
{
...

    {
        ...
        for (CTxMemPool::txiter it : allConflicting)
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s AUS additional fees, %d delta bytes\n",
...
static bool ReadBlockOrHeader(T& block, const CDiskBlockPos& pos, const Consensus::Params& consensusParams)
{
    ...
    // A different rule for the genesis block. Because the original AUS genesis wan not mined correctly.
        ...
        if (fScriptChecks) {
            ...
            for (unsigned int i = 0; i < tx.vin.size(); i++) {
                ...
                if (pvChecks) {
                    ...
                    // AUS-DEV note: that softfork is not the case of AUS network - SANDO re-enabled
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // AUS dev note: some valid blocks fails this test so temporary bypass it untill resolve the reason
    // only log a warnings
    if (!control.Wait())
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    // AustraliaCash: Disallow AuxPow blocks before it is activated.
    ...
    // AustraliaCash: Version 2 enforcement was never used
...
static bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev)
{
    ...
    // AustraliaCash: We probably want to disable this
    int nLockTimeFlags = 0;
