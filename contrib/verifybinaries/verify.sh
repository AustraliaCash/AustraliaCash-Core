...
###   australiacashcore.org and australiacash.org and compares them.
...
WORKINGDIR="/tmp/australiacash_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://australiacashcore.org"
HOST2="https://australiacash.org"
BASEDIR="/bin/"
VERSIONPREFIX="australiacash-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'australiacash-',
...
if [ $? -ne 0 ]; then
   echo "australiacash.org failed to provide signature file, but australiacashcore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "australiacash.org and australiacashcore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the AustraliaCash Core binary release signing key installed?"
   fi
