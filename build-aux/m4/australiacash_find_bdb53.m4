AC_DEFUN([AUSTRALIACASH_FIND_BDB53],[
  ...
  if test "x$bdbpath" = "xX"; then
    AC_MSG_RESULT([no])
    AC_MSG_ERROR([libdb_cxx headers missing, AustraliaCash Core requires this library for wallet functionality (--disable-wallet to disable wallet functionality)])
  elif test "x$bdb53path" = "xX"; then
    AUSTRALIACASH_SUBDIR_TO_INCLUDE(BDB_CPPFLAGS,[${bdbpath}],db_cxx)
    ...
    AUSTRALIACASH_SUBDIR_TO_INCLUDE(BDB_CPPFLAGS,[${bdb53path}],db_cxx)
    bdbpath="${bdb53path}"
  fi
  ...
  if test "x$BDB_LIBS" = "x"; then
      AC_MSG_ERROR([libdb_cxx missing, AustraliaCash Core requires this library for wallet functionality (--disable-wallet to disable wallet functionality)])
  fi
  AC_SUBST(BDB_LIBS)
])
