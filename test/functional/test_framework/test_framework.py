...
class AustraliaCashTestMetaClass(type):
    """Metaclass for AustraliaCashTestFramework.

    Ensures that any attempt to register a subclass of `AustraliaCashTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'AustraliaCashTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("AustraliaCashTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("AustraliaCashTestFramework subclasses may not override "
                                "'__init__' or 'main'")
